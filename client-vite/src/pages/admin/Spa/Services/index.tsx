...
const SpaServiceList: React.FC = () => {
    ...
    const fetchServices = useCallback(async () => {
        ...
        try {
            ...
            setMessage({ type: 'error', text: 'Tải danh sách dịch vụ spa thất bại.' });
            setServices([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleFormSubmit = async (
        ...
        if (error) {
            const errorMessage = typeof error === 'string' ? error : 
                (editingService ? 'Cập nhật dịch vụ spa thất bại.' : 'Thêm dịch vụ spa thất bại.');
        ...
        if (!isLoadingFromChild && data) {
            setMessage({ 
                type: 'success', 
                text: editingService ? 'Dịch vụ spa đã được cập nhật thành công!' : 'Dịch vụ spa đã được thêm thành công!' 
            });
            setEditingService(null);
            setIsModalVisible(false);
            fetchServices();
    ...
    const handleDelete = async (serviceId: string) => {
        setLoading(true);
        try {
            await spaServiceService.deleteSpaService(serviceId);
            setMessage({ type: 'success', text: 'Dịch vụ spa đã được xóa thành công!' });
            fetchServices();
        } catch (error) {
            console.error("Error delete spa service:", error);
            setMessage({ type: 'error', text: 'Xóa dịch vụ spa thất bại.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const columns: TableColumnsType<SpaService> = [
        {
            title: 'Hình ảnh',
            dataIndex: 'imagePath',
            key: 'imagePath',
