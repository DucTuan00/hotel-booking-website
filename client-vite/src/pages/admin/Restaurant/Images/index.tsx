...
import ImageItems from '@/pages/admin/Restaurant/Images/ImageItems';

const { Title } = Typography;

const RestaurantImageList: React.FC = () => {
    const [existingImages, setExistingImages] = useState<RestaurantImage[]>([]);
    ...
    const fetchImages = useCallback(async () => {
        setLoading(true);
        try {
            const data = await restaurantImageService.getAllRestaurantImages();
            setExistingImages(data.images as RestaurantImage[]);
        } catch (error) {
            console.error("Error when fetch restaurant images:", error);
            setMessage({ type: 'error', text: 'Tải danh sách ảnh nhà hàng thất bại.' });
            setExistingImages([]);
        } finally {
            setLoading(false);
        }
        ...
        try {
            ...
            if (newImages.length > 0) {
                ...
                for (const imagePath of uploadedPaths) {
                    await restaurantImageService.createRestaurantImage({
                        imagePath,
                        title: '',
                        description: ''
                    });
            ...
            if (deletedImageIds.length > 0) {
                for (const imageId of deletedImageIds) {
                    await restaurantImageService.deleteRestaurantImage(imageId);
                }
            }
    ...
    return (
        <div style={{ padding: '4px' }}>
            <div style={{ 
                ...
                <Title level={2} style={{ margin: 0 }}>Quản lý ảnh nhà hàng</Title>
            ...
            {visibleExistingImages.length === 0 && newImages.length === 0 && !loading ? (
                <div style={{
                    ...
                    <div style={{ fontSize: '16px', color: '#999' }}>
                        Chưa có ảnh nhà hàng nào. Hãy thêm ảnh đầu tiên!
                    </div>
                </div>
            ) : (
                <Row gutter={[16, 16]}>
