...
export const buildItineraryPrompt = (
    ...
    prompt += `\n## Các địa điểm gợi ý (THAM KHẢO, có thể bổ sung thêm địa điểm khác):\n`;
    if (interests) {
        interests.forEach(interest => {
...
- Danh sách địa điểm ở trên chỉ là GỢI Ý THAM KHẢO, KHÔNG phải danh sách bắt buộc
- AI được KHUYẾN KHÍCH bổ sung các địa điểm NỔI TIẾNG KHÁC tại Hà Nội phù hợp với sở thích đã chọn
- TUYỆT ĐỐI KHÔNG lặp lại cùng một địa điểm trong toàn bộ kế hoạch
- Mỗi hoạt động phải là địa điểm/trải nghiệm KHÁC NHAU
- Ưu tiên đa dạng: kết hợp địa điểm nổi tiếng + địa điểm ít người biết (hidden gems)
- VÍ DỤ địa điểm bổ sung: Phố sách Đinh Lễ, Bảo tàng Mỹ thuật, Chợ Hôm, Phố Hàng Mã, Quán cà phê vintage, Phố Nhà Thờ, West Lake Water Park, Aeon Mall Long Biên...
